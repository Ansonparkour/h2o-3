...
public class AstCorrelation extends AstPrimitive {
  ...
  private Val array(Frame frx, Frame fry, Mode mode) {
    ...
    if (mode.equals(Mode.CompleteObs)) {
      ...
      double[] ymeansComplete = ArrayUtils.div(taskCompleteObsMean._ysum, fry.numRows() - NACount);
      double[] xmeansComplete = ArrayUtils.div(taskCompleteObsMean._xsum, fry.numRows() - NACount);

      CorTask[] cvs = new CorTask[ncoly];

      // Launch tasks; each does all Xs vs one Y
      for (int y = 0; y < ymeansComplete.length; y++)
        cvs[y] = new CorTask(ymeansComplete[y], xmeansComplete,true).dfork(new Frame(vecys[y]).add(frx));
    ...
    // Launches tasks. Each does all Xs vs one Y
    for (int y = 0; y < ncoly; y++)
      cvs[y] = new CorTask(vecys[y].mean(), xmeans,false).dfork(new Frame(vecys[y]).add(frx));

    // 1-col will returns a scalar
    if (ncolx == 1 && ncoly == 1) {
      return new ValNum(cvs[0].getResult()._cors[0] / (cvs[0].getResult()._denom[0]));
    }

    // Gather all the Xs-vs-Y covariance arrays; divide by (sigma(x) * sigma(y))
    Vec[] res = new Vec[ncoly];
    Key<Vec>[] keys = Vec.VectorGroup.VG_LEN1.addVecs(ncoly);
    for (int y = 0; y < ncoly; y++)
      res[y] = Vec.makeVec(ArrayUtils.div(cvs[y].getResult()._cors, (cvs[y].getResult()._denom)), keys[y]);
